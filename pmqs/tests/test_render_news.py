...
def test_news_evidence_rendered_hedged():
    """Hedged-news rendering ('reportedly, via <source>') lives in source_card_html,
    used by the Inbox detail pane. The Workspace Evidence tab that used to also show
    this is removed for now (dogfooding feedback) -- _evidence_html stays defined,
    parked, so this still covers the hedging behaviour it would render."""
    from pmqs.web.render import _evidence_html

    ev = [{"type": "news", "source": "TechCrunch", "title": "Rival raises $50M",
          "url": "http://tc/1", "date": "2026-07-14", "hedged": True}]
    html = _evidence_html(ev)
